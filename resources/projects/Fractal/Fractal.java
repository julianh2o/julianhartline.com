...
public class Fractal extends Applet implements Runnable {
	...
	public void sendGif() {
		try {
			debug("Creating gif...");
			debug("Frames: "+fractalSteps);
			paint(getGraphics());
			debug("Calculating fractal size..");
			paint(getGraphics());
			for (int i=0; i<=fractalSteps*2; i++) {
				paintFractal(offscreen,i);
			}
			URL url = new URL(getCodeBase() + "post.php");
			...
			int border = 5;
			BufferedImage wholefractal = new BufferedImage(width,height,BufferedImage.TYPE_3BYTE_BGR);
			BufferedImage clipped = new BufferedImage(fwidth+2*border,fheight+2*border,BufferedImage.TYPE_3BYTE_BGR);
			Graphics gfrac = wholefractal.createGraphics();
			Graphics gclip = clipped.createGraphics();
			if (animate) {
				ge.setRepeat(0);
				int tdelay = delay.getInt(0);
				int delay = tdelay;
				if (delay < 50) delay = 50;
				ge.setDelay(delay);
				for (int t=0; t<tdelay*fractalSteps*2; t+=delay) {
					int i = t/tdelay;
					gfrac.clearRect(0,0,width,height);
					paintFractal(gfrac,i);
					gclip.drawImage(wholefractal,border,border,fwidth,fheight,fminx,fminy,fmaxx,fmaxy,this);
					cstep = i;
					paint(getGraphics());
					ge.addFrame(clipped);
				}
			...
			//believe it or not, it doesnt work without reading... 
			...
			out.close();
			in.close();
			debug("Saved to: "+getCodeBase().toString()+"images/"+acc+".gif");
			popupWindow(getCodeBase().toString()+"images/"+acc+".gif");
		} catch (Exception e) {
			StringWriter sw = new StringWriter();
			e.printStackTrace(new PrintWriter(sw));
			debug = sw.toString();
